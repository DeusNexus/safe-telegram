...
bot.hears(new RegExp(/\/init/s), async ctx => {
    ...
    if(!res) {
        ...
        //USER ALREADY INIT
        ctx.replyWithPhoto({ source: 'init.png' }, Extra.caption('Your SAFE wallet is already initialized, you can only do this once!').HTML())
    }
})
...
bot.hears(new RegExp(/\/secret/s), async ctx => {
    ...
    if(res) {
        ...
        ctx.replyWithPhoto({source: 'safe.jpg'}, Extra.caption(text).HTML())
    } else {
        console.log("USER:",res)
        ctx.replyWithHTML(initMessage)
    }
})
...
bot.hears('/send', ctx => {
    ctx.replyWithHTML(`Try <code>/send [amount] [Receive SAFE URL]</code>, \nExample: <code>/send 25 safe://hbyyyybkyosnrn1ir1fqcxks9r16xe83dgjimxdo6p18wugfqff9yy6p3w</code>`)
})
    ...
    if(res){
        ...
        if(receiver.id > 0){
            exec(`safe keys transfer --from ${user.sk_wallet} --to ${toSafeURL} ${amount} --json`, (error, stdout, stderr) => {
                if (error) {
                    console.log(`error: ${error.message}`);
                    ctx.replyWithHTML('The SAFE URL doesnt work!')
                    return;
                }
                if (stderr) {
                    console.log(`stderr: ${stderr}`);
                ...
                //RECEIVER ID
                ctx.telegram.sendMessage(receiver.id, `You <b>received</b> <code>${amount}</code> SAFE Coin(s) from ${nickname}!`, {parse_mode: 'HTML'})
            });    
        } else {
            ctx.replyWithHTML('The SAFE URL you try to sent to is not connected to any user!')
        }
    } else {
        //USER HAS NOT INIT
        ctx.replyWithHTML(initMessage)
    ...
    if(res_sender) {
        ...
        if(res_receiver) {
            ...
            exec(`safe keys transfer --from ${sender.sk_wallet} --to ${receiver.safeurl_wallet} ${amount} --json`, (error, stdout, stderr) => {
                if (error) {
                    console.log(`error: ${error.message}`);
                    ctx.replyWithHTML('Unable to tip person')
                    return;
                }
                if (stderr) {
                    console.log(`stderr: ${stderr}`);
                ...
                console.log(`stdout: ${stdout}`);
                // const obj = JSON.parse(stdout)
                ctx.replyWithHTML(`You <b>tipped</b> <i>${nicknameReceiver}</i> a total of <code>${amount}</code> <b>SAFE Coins</b> to his/her wallet!`)
                ctx.telegram.sendMessage(id, `You <b>received</b> <code>${amount}</code> <b>SAFE Coins</b> as a tip from ${nicknameSender}!`, {parse_mode: 'HTML'})
            });            
        } else {
            //NOT IN DB
    ...
    exec(`safe cat ${param} --json`, (error, stdout, stderr) => {
        ...
        try {
            if(Array.isArray(JSON.parse(stdout))) {
                ...
                ctx.replyWithHTML(`<b>[File Content]:</b> <code>${stdout}</code>`)
            }
        } catch(e) {
            ctx.replyWithHTML(`<b>[File Content]:</b> <code>${stdout}</code>`)
        }
...
bot.hears(new RegExp(/\/dog\ssafe:\/\/(.*)/s), ctx => {
    ...
    exec(`safe dog ${param} --json`, (error, stdout, stderr) => {
        ...
        if(keyType === 'FilesContainer') {
            ...
            ctx.reply("No Data for this type!")
        }
